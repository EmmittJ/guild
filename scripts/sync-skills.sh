...
# Finds the repo root by walking up from CWD until it finds a .git entry (works with worktrees).
...
if [ ! -e "$REPO_ROOT/.git" ]; then
  echo "Error: could not find repo root (no .git found in any parent directory)" >&2
  exit 1
fi
...
SKILL_ENTRIES="orchestrate:plugin/skills/orchestrate train-agent:plugin/skills/train-agent train-skill:plugin/skills/train-skill work-cycle:plugin/skills/work-cycle"
