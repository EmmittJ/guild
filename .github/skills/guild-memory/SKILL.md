...
The memory root for this repo is `.guild/memory` (relative to repo root).
...
**Format:** `YYYY-MM-DD-{slug}.md` in `.guild/memory/decisions/`
...
- After writing a decision, update `.guild/memory/decisions/_summary.md` with a one-liner
...
**Format:** One file per domain in `.guild/memory/insights/`
...
**Format:** `.guild/memory/context/{session-id}.md` — one file per session, named by short UUID (e.g. `a3f9c1b2`)
